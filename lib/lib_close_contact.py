...
def get_close_contact(es, row, timezone, d = '10m', index_prefix = '', filtering = 2):
    local_time = row['acquisitionTime'].tz_localize(tz ='UTC').tz_convert(tz = timezone)
    index_name = index_prefix + local_time.strftime('%m_%d').lower() # get the index time to query!
...
def track_close_contact(file_, person_type, output_folder, minutes_before = 3, minutes_after = 3, distance = '10m',
    ...
    df['acquisitionTime'] = pd.to_datetime(df['acquisitionTime'],utc =True).dt.tz_convert(tz = None) # convert to utc and remove timezone info
    ...
    close_contact = pd.DataFrame(df.apply(lambda row: get_close_contact(es, row, time_zone, d = distance, filtering = filtering,
